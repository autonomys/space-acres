...
node_migration_mode_fresh_sync = Synchronisation complète vers un nouvel emplacement
node_migration_mode_fresh_sync_explanation = Synchronise une base de données vierge depuis le réseau vers le nouvel emplacement. Souvent plus rapide que la migration, surtout si votre nœud est désynchronisé, et nécessite moins d’espace de destination.
...
node_migration_mode_reset = Réinitialiser et resynchroniser localement
node_migration_mode_reset_explanation = Réinitialise votre nœud en effaçant la base de données et en resynchronisant depuis le réseau. À utiliser si la base de données de votre nœud est corrompue ou fortement désynchronisée.
...
error_button_reset_node_tooltip = Effacer les données du nœud et resynchroniser depuis le réseau
...
notification_farm_error = L'une des fermes de Space Acres a rencontré une erreur
    .body = Une erreur est survenue et nécessite une intervention de l'utilisateur pour la résoudre
notification_node_low_disk_space = Espace disque faible sur le nœud
    .body = Le volume du nœud ne dispose plus que de {$free_space}
...
warning_low_disk_space_detail = Il ne reste que {$free_space}. Envisagez une migration vers un disque plus grand.
