...
configuration_node_size = Düğüm boyutu: {$size}
...
node_migration_mode_fresh_sync = Yeni konuma yeni senkronizasyon
node_migration_mode_fresh_sync_explanation = Yeni konumda ağdan yeni bir veritabanı senkronize eder. Genellikle taşımaktan daha hızlı, özellikle node'unuz senkronize değilse, ve hedef konumda daha az alan gerektirir.
...
node_migration_mode_reset_explanation = Veritabanını silerek ve ağdan yeniden senkronize ederek node'unuzu sıfırlar. Node veritabanınız bozuksa veya önemli ölçüde senkronize değilse bunu kullanın.
node_migration_non_node_data_warning = Not: Bu dizinde düğüm dışı veriler tespit edildi ve taşınmayacak
...
node_migration_status_updating_config = Konfigürasyon güncelleniyor...
node_migration_status_verifying = Veritabanı doğrulanıyor...
node_migration_title = Node Veritabanı Taşınıyor
...
error_button_reset_node = Düğümü sıfırla
error_button_reset_node_tooltip = Düğüm verilerini sil ve ağdan yeniden senkronize et
...
notification_node_low_disk_space = Düşük Node Disk Alanı
    .body = Node biriminde sadece {$free_space} kaldı
...
warning_low_disk_space = Node biriminde düşük disk alanı
